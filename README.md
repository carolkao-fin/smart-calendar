...
**一天會排進好幾件事，但每件都是做得動的一塊**

預設的「平均分攤」讓當天的容量被幾件事分掉，而不是最急的那件吃光整天。
每件事排到某一天，就至少給「每件事每天至少排」那麼多（預設 2 小時），
密度更高才給更多 —— 所以不會出現一天被切成一堆 30 分鐘碎塊的情況。

一天大約排幾件，是這兩個設定算出來的：

```
一天排幾件 ≈ 每日可投入時數 ÷ 每件事每天至少排     （預設 4 ÷ 2 = 2 件）
```

想一天碰到更多件，就把「每件事每天至少排」調小或把「每日可投入時數」調大；
設定畫面上會直接顯示目前的組合大約是幾件。
想回到「一件一件做完」，把排程策略改成「儘早做完」。
...
今日面板打卡完的那幾列也會留在原地（劃掉、附「恢復」），按錯當場就能收回，
不必先切到完成紀錄頁。
...
| 每件事每天至少排 | 2h | 排到某天就至少給這麼多，決定一天大約排幾件 |
| 工作日 | 週一～週五 | 取消勾選就不排那天 |
...
排程每次都從今天重算，所以「今天以後」的格子畫的是計畫；
**今天以前的格子畫的是實際發生過的事**，往回翻月份都還在：

- `✓ 事情名稱 3h` — 那天打卡或結案的紀錄，任務後來被刪掉也還留著
- 格子右上角的合計與底部細條，過去那幾天算的是**實際做掉的時數**，用完成色顯示
- 紅色的 `逾期 事情名稱` — 那天到期卻還沒結案的事，留在原本的日期上不會消失
- 虛線紅框的區塊 — 時間已經過了的固定時段（開會、看診）還沒標記完成

**未完成清單**

日曆右上角切到「紀錄」，最上面就是**截止日已經過了卻還沒結案**的事，
「紀錄」鍵上的紅色數字就是它的件數。每一列有三條路：

| 按鈕 | 做什麼 |
|---|---|
| 補完成 | 其實那天就做完了，只是忘了勾。紀錄會**記在原本的截止日那天**，行事曆上那格立刻看得到 |
| 編輯 | 還要做，改期或調整時數 |
| 刪除 | 不做了。任務從清單與行事曆消失，**已經留下的完成紀錄不受影響** |

補錯了也不要緊：到下方的紀錄按「恢復」，那件事就回到未完成清單。

**完成紀錄**

同一頁的下半部是每天實際做完了什麼：

- 上方五格：未完成件數、今日完成時數、本週完成時數、今日結案件數、累計結案件數
- 下方依日期由新到舊分段，每天標出當日總時數與筆數
- 每一筆都可以「恢復」，還原成那筆紀錄之前的狀態

紀錄有兩種來源：今日面板按「完成 Xh」的**打卡**，和勾選完成的**結案**。
它同時是行事曆上那些過去日子的唯一來源，所以保留最近 2000 筆才會開始丟最舊的。
...
```
python test_browser.py        # 網頁（80 項）
...
也會實際去點勾選框、打卡、補完成、恢復與刪除鍵，
驗證重複任務、完成紀錄、未完成清單與行事曆上的過去。
